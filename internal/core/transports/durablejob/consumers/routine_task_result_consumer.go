...
func (c *DurableJobRoutineTaskResultConsumer) Start(ctx context.Context) func() {
	...
	if err != nil {
		if logs.NotegicLogger != nil {
			logs.NotegicLogger.Error(ctx, err, "Failed to create DurableJob routine task result consumer")
		}
		return func() {}
	}
	...
	go func() {
		if err := consumer.Run(workerCtx, c.consume); err != nil && workerCtx.Err() == nil && logs.NotegicLogger != nil {
			logs.NotegicLogger.Error(workerCtx, err, "DurableJob routine task result consumer stopped")
		}
	}()
