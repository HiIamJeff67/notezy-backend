...
func (t *SubShelfEndpoint) SearchSubShelves(ctx *gin.Context) {
	request := &gatewaycontract.Request[apicontract.SearchSubShelvesRequestDto]{}
	...
	ctx.JSON(http.StatusOK, gatewaycontract.Response[apicontract.SearchSubShelvesResponseDto]{
		Version: gatewaycontract.Version,
		Metadata: gatewaycontract.ResponseMetadata{
			RequestId:   request.Metadata.RequestId,
			RespondedAt: time.Now(),
