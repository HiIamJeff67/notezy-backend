...
func NewRateLimitRecordCacheClient() *RateLimitRecordCacheClient {
	rangeValue := types.Range[int, int]{Start: 9, Size: 4}

	return &RateLimitRecordCacheClient{
		Range:           rangeValue,
		MaxServerNumber: rangeValue.Start + rangeValue.Size - 1,
		backendServerNameToRedisIndex: map[platformredis.BackendServerName]int{
			platformredis.BackendServerName_EastAsia:    9,
			platformredis.BackendServerName_EastAmerica: 10,
			platformredis.BackendServerName_WestAmerica: 11,
			platformredis.BackendServerName_WestEurope:  12,
		},
