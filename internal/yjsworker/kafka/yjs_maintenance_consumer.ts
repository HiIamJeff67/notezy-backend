...
} from "./core_command_dispatcher.js";
...
export class YjsMaintenanceConsumer {
  ...
  private async execute(command: MaintenanceCommand): Promise<MaintenanceResult> {
  ...
  private async executeCompaction(command: MaintenanceCommand): Promise<MaintenanceResult> {
    ...
    const input = parseYjsCompactionInput(Buffer.from(loaded.payload, "base64"));
    ...
    return {
      ...
      compactedUntilSequence: applied.applied ? input.cutoffSequence : input.baseCompactedUntilSequence,
      projectedUntilSequence: -1,
    };
  }

  private async executeProjection(command: MaintenanceCommand): Promise<MaintenanceResult> {
    const loaded = await this.dispatcher.dispatch<
      Record<string, never>,
      { found: boolean; payload?: string }
