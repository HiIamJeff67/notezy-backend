...
func (c *YjsMaintenanceHintConsumer) Start(ctx context.Context) func() {
	consumer, err := platformkafka.NewConsumer(
		c.kafkaConfig,
		durablejobeventscontract.CoreDurableJobYjsMaintenanceHintTopic.String(),
	)
	if err != nil {
...
func (c *YjsMaintenanceHintConsumer) consume(
	...
	if event.EventType != durablejobeventscontract.EventType_YjsMaintenanceHint ||
	...
	var hint durablejobeventscontract.YjsMaintenanceHintData
...
func (c *YjsMaintenanceHintConsumer) dispatchPending(ctx context.Context) {
	for {
		...
		for _, hint := range hints {
			c.slots <- struct{}{}
			waitGroup.Add(1)
			go func(hint durablejobeventscontract.YjsMaintenanceHintData) {
				defer waitGroup.Done()
				defer func() { <-c.slots }()
...
func (c *YjsMaintenanceHintConsumer) retryHint(ctx context.Context, hint durablejobeventscontract.YjsMaintenanceHintData) {
	go func() {
...
func (c *YjsMaintenanceHintConsumer) dispatchHint(ctx context.Context, hint durablejobeventscontract.YjsMaintenanceHintData) error {
	startedAt := time.Now()
	operation := durablejobeventscontract.YjsMaintenanceOperation_Project
	if hint.UncompactedUpdateCount >= yjsworkercontract.YjsCompactionUpdateThreshold ||
		...
		operation = durablejobeventscontract.YjsMaintenanceOperation_Compact
	}
	if operation == durablejobeventscontract.YjsMaintenanceOperation_Project && hint.ProjectedUntilSequence >= hint.LatestUpdateSequence {
		if metrics.NotezyMeter != nil {
