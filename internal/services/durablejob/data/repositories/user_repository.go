...
func (r *UserRepository) GetOneById(
	...
	if exception := exceptions.Cover(nil, []types.Pair[bool, *exceptions.Exception]{
		{First: result.Error != nil, Second: durablejobexceptions.User.NotFound().WithOrigin(result.Error)},
		{First: user.Id == uuid.Nil, Second: durablejobexceptions.User.NotFound()},
	}); exception != nil {
...
func (r *UserRepository) GetAll(
	...
	if exception := exceptions.Cover(nil, []types.Pair[bool, *exceptions.Exception]{
		{First: result.Error != nil, Second: durablejobexceptions.User.NotFound().WithOrigin(result.Error)},
		{First: len(users) == 0, Second: durablejobexceptions.User.NotFound()},
	}); exception != nil {
		return nil, exception
	}
	return users, nil
...
func (r *UserRepository) CreateOne(
	...
	if err := copier.Copy(&newUser, &input); err != nil {
		return nil, durablejobexceptions.User.FailedToCreate().WithOrigin(err)
	}
	...
	if err := result.Error; err != nil {
		...
		case "ERROR: duplicate key value violates unique constraint \"uni_UserTable_name\" (SQLSTATE 23505)":
			return nil, durablejobexceptions.User.DuplicateName(input.Name)
		case "ERROR: duplicate key value violates unique constraint \"uni_UserTable_email\" (SQLSTATE 23505)":
			return nil, durablejobexceptions.User.DuplicateEmail(input.Email)
		default:
			return nil, durablejobexceptions.User.FailedToCreate() // .WithOrigin(err) <- don't show the database error to outside
		}
	}
	if result.RowsAffected == 0 {
		...
		return nil, durablejobexceptions.User.NoChanges()
	}
...
func (r *UserRepository) UpdateOneById(
	...
	result := parsedOptions.DB.Model(&schemas.User{}).
		...
		Updates(&updates)
	if exception := exceptions.Cover(nil, []types.Pair[bool, *exceptions.Exception]{
		{First: result.Error != nil, Second: durablejobexceptions.User.FailedToUpdate().WithOrigin(result.Error)},
		{First: result.RowsAffected == 0, Second: durablejobexceptions.User.NoChanges()},
	}); exception != nil {
		return nil, exception
	}
