...
func (r *UsersToShelvesRepository) GetOne(
	...
	result := parsedOptions.DB.
		...
		First(&relation)
	if result.Error != nil {
		return nil, durablejobexceptions.Shelf.NotFound().WithOrigin(result.Error)
	}
...
func (r *UsersToShelvesRepository) UpsertMany(
	...
	if len(userIds) != len(permissions) {
		return nil, durablejobexceptions.Shelf.InvalidInput("userIds and permissions must have equal lengths")
	}
...
func (r *UsersToShelvesRepository) UpdateOne(
	...
	if result.RowsAffected == 0 {
		return nil, durablejobexceptions.Shelf.NotFound()
	}
...
func (r *UsersToShelvesRepository) DeleteOne(
	...
	if result.Error != nil {
		return durablejobexceptions.Shelf.FailedToDelete().WithOrigin(result.Error)
	}
	if result.RowsAffected == 0 {
