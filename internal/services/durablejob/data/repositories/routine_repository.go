...
func (r *RoutineRepository) CheckPermissionAndGetOneById(
	...
	if exception := exceptions.Cover(nil, []types.Pair[bool, *exceptions.Exception]{
		{First: result.Error != nil, Second: durablejobexceptions.Routine.NotFound().WithOrigin(result.Error)},
		{First: routine.Id == uuid.Nil, Second: durablejobexceptions.Routine.NotFound()},
	}); exception != nil {
...
func (r *RoutineRepository) CheckPermissionsAndGetManyByIds(
	...
	if exception := exceptions.Cover(nil, []types.Pair[bool, *exceptions.Exception]{
		{First: result.Error != nil, Second: durablejobexceptions.Routine.NotFound().WithOrigin(result.Error)},
		{First: len(routines) == 0, Second: durablejobexceptions.Routine.NotFound()},
	}); exception != nil {
		return nil, exception
	}
...
func (r *RoutineRepository) GetAllByTimeRange(
	...
	result := parsedOptions.DB.
		...
		Find(&routines)
	if result.Error != nil {
		return nil, durablejobexceptions.Routine.NotFound().WithOrigin(result.Error)
	}
...
func (r *RoutineRepository) CreateOneByStationId(
	...
	if !stationRepository.HasPermission(
		...
		return nil, durablejobexceptions.Routine.NoPermission("create a routine under this station")
	}
	...
	if err := copier.Copy(&newRoutine, &input); err != nil {
		parsedOptions.DB.Rollback()
		return nil, durablejobexceptions.Routine.InvalidInput().WithOrigin(err)
	}
	...
	result := parsedOptions.DB.Model(&schemas.Routine{}).
		Create(&newRoutine)
	if exception := exceptions.Cover(nil, []types.Pair[bool, *exceptions.Exception]{
		{First: result.Error != nil, Second: durablejobexceptions.Routine.FailedToCreate().WithOrigin(result.Error)},
		{First: result.RowsAffected == 0, Second: durablejobexceptions.Routine.NoChanges()},
	}); exception != nil {
		parsedOptions.DB.Rollback()
		return nil, exception
	}
	...
	if shouldStartTransaction {
		if err := parsedOptions.DB.Commit().Error; err != nil {
			parsedOptions.DB.Rollback()
			return nil, durablejobexceptions.Routine.FailedToCommitTransaction().WithOrigin(err)
		}
	}
...
func (r *RoutineRepository) CreateManyByStationIds(
	...
	if len(input) == 0 {
		return nil, durablejobexceptions.Routine.NoChanges()
	}
...
func (r *RoutineRepository) UpdateOneById(
	...
	if input.Values.StationId != nil && !partialupdate.CheckSetNull(input.SetNull, "StationId") {
		...
		if !stationRepository.HasPermission(
			*input.Values.StationId,
			...
			parsedOptions.DB.Rollback()
			return nil, durablejobexceptions.Routine.NoPermission("move a routine to this station")
		}
	}
	...
	result := parsedOptions.DB.Model(&schemas.Routine{}).
		...
		Updates(&updates)
	if exception := exceptions.Cover(nil, []types.Pair[bool, *exceptions.Exception]{
		{First: result.Error != nil, Second: durablejobexceptions.Routine.FailedToUpdate().WithOrigin(result.Error)},
		{First: result.RowsAffected == 0, Second: durablejobexceptions.Routine.NoChanges()},
	}); exception != nil {
		parsedOptions.DB.Rollback()
		return nil, exception
...
func (r *RoutineRepository) UpdateManyByIds(
	...
	if len(input) == 0 {
		return durablejobexceptions.Routine.NoChanges()
	}
	...
	if exception != nil {
		parsedOptions.DB.Rollback()
		return durablejobexceptions.Routine.NoPermission("update these routines")
	}
	...
	if len(targetStationIdSet) > 0 {
		...
		if !stationRepository.HavePermissions(
			...
			parsedOptions.DB.Rollback()
			return durablejobexceptions.Routine.NoPermission("move these routines to the given stations")
		}
	}
	...
	if shouldStartTransaction {
		if err := parsedOptions.DB.Commit().Error; err != nil {
			parsedOptions.DB.Rollback()
			return durablejobexceptions.Routine.FailedToCommitTransaction().WithOrigin(err)
		}
	}
...
func (r *RoutineRepository) RestoreSoftDeletedOneById(
	...
	if exception := exceptions.Cover(nil, []types.Pair[bool, *exceptions.Exception]{
		{First: result.Error != nil, Second: durablejobexceptions.Routine.FailedToUpdate().WithOrigin(result.Error)},
		{First: restoredRoutine.Id == uuid.Nil, Second: durablejobexceptions.Routine.FailedToUpdate()},
		{First: result.RowsAffected == 0, Second: durablejobexceptions.Routine.NoChanges()},
	}); exception != nil {
		return nil, exception
	}
...
func (r *RoutineRepository) RestoreSoftDeletedManyByIds(
	...
	if exception := exceptions.Cover(nil, []types.Pair[bool, *exceptions.Exception]{
		{First: result.Error != nil, Second: durablejobexceptions.Routine.FailedToUpdate().WithOrigin(result.Error)},
		{First: len(restoredRoutines) == 0, Second: durablejobexceptions.Routine.FailedToUpdate()},
		{First: result.RowsAffected == 0, Second: durablejobexceptions.Routine.NoChanges()},
	}); exception != nil {
		return nil, exception
	}
...
func (r *RoutineRepository) HardDeleteOneById(
	...
	if exception := exceptions.Cover(nil, []types.Pair[bool, *exceptions.Exception]{
		{First: result.Error != nil, Second: durablejobexceptions.Routine.FailedToDelete().WithOrigin(result.Error)},
		{First: result.RowsAffected == 0, Second: durablejobexceptions.Routine.NoChanges()},
	}); exception != nil {
		return exception
	}
...
func (r *RoutineRepository) BulkCheckPermissionsAndGetManyByIds(
	...
	result := parsedOptions.DB.Model(&schemas.Routine{}).
		...
		Scan(&validTargets)
	if result.Error != nil {
		return nil, nil, durablejobexceptions.Routine.NotFound().WithOrigin(result.Error)
	}
...
func (r *RoutineRepository) BulkCreateMany(
	...
	if len(inputs) == 0 {
		return []bool{}, durablejobexceptions.Routine.NoChanges()
	}
	...
	if result.Error != nil {
		parsedOptions.DB.Rollback()
		return nil, durablejobexceptions.Routine.FailedToCreate().WithOrigin(result.Error)
	}
...
func (r *RoutineRepository) BulkUpdateMany(
	...
	if len(targetStationIds) > 0 {
		...
		if result.Error != nil {
			parsedOptions.DB.Rollback()
			return nil, durablejobexceptions.Routine.FailedToUpdate().WithOrigin(result.Error)
		}
