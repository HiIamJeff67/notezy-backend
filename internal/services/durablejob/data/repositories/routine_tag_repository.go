...
func (r *RoutineTagRepository) GetOneById(
	...
	if exception := exceptions.Cover(nil, []types.Pair[bool, *exceptions.Exception]{
		{First: result.Error != nil, Second: durablejobexceptions.RoutineTag.NotFound().WithOrigin(result.Error)},
		{First: routineTag.Id == uuid.Nil, Second: durablejobexceptions.RoutineTag.NotFound()},
	}); exception != nil {
...
func (r *RoutineTagRepository) GetManyByIds(
	...
	if exception := exceptions.Cover(nil, []types.Pair[bool, *exceptions.Exception]{
		{First: result.Error != nil, Second: durablejobexceptions.RoutineTag.NotFound().WithOrigin(result.Error)},
		{First: len(routineTags) == 0, Second: durablejobexceptions.RoutineTag.NotFound()},
	}); exception != nil {
		return nil, exception
	}
...
func (r *RoutineTagRepository) GetAllByUserId(
	...
	result := parsedOptions.DB.
		...
		Find(&routineTags)
	if result.Error != nil {
		return nil, durablejobexceptions.RoutineTag.NotFound().WithOrigin(result.Error)
	}
...
func (r *RoutineTagRepository) CreateOne(
	...
	if err := copier.Copy(&newRoutineTag, &input); err != nil {
		...
		return nil, durablejobexceptions.RoutineTag.InvalidInput().WithOrigin(err)
	}
	...
	if exception := exceptions.Cover(nil, []types.Pair[bool, *exceptions.Exception]{
		{First: result.Error != nil, Second: durablejobexceptions.RoutineTag.FailedToCreate().WithOrigin(result.Error)},
		{First: result.RowsAffected == 0, Second: durablejobexceptions.RoutineTag.NoChanges()},
	}); exception != nil {
		parsedOptions.DB.Rollback()
		return nil, exception
	}
	...
	if shouldStartTransaction {
		if err := parsedOptions.DB.Commit().Error; err != nil {
			parsedOptions.DB.Rollback()
			return nil, durablejobexceptions.RoutineTag.FailedToCommitTransaction().WithOrigin(err)
		}
	}
...
func (r *RoutineTagRepository) CreateMany(
	...
	if len(input) == 0 {
		return nil, durablejobexceptions.RoutineTag.NoChanges()
	}
...
func (r *RoutineTagRepository) UpdateOneById(
	...
	result := parsedOptions.DB.
		...
		Updates(&updates)
	if exception := exceptions.Cover(nil, []types.Pair[bool, *exceptions.Exception]{
		{First: result.Error != nil, Second: durablejobexceptions.RoutineTag.FailedToUpdate().WithOrigin(result.Error)},
		{First: result.RowsAffected == 0, Second: durablejobexceptions.RoutineTag.NoChanges()},
	}); exception != nil {
		parsedOptions.DB.Rollback()
		return nil, exception
...
func (r *RoutineTagRepository) UpdateManyByIds(
	...
	if len(input) == 0 {
		return durablejobexceptions.RoutineTag.NoChanges()
	}
	...
	if exception != nil {
		parsedOptions.DB.Rollback()
		return durablejobexceptions.RoutineTag.NotFound()
	}
	...
	if shouldStartTransaction {
		if err := parsedOptions.DB.Commit().Error; err != nil {
			parsedOptions.DB.Rollback()
			return durablejobexceptions.RoutineTag.FailedToCommitTransaction().WithOrigin(err)
		}
	}
...
func (r *RoutineTagRepository) HardDeleteOneById(
	...
	if exception := exceptions.Cover(nil, []types.Pair[bool, *exceptions.Exception]{
		{First: result.Error != nil, Second: durablejobexceptions.RoutineTag.FailedToDelete().WithOrigin(result.Error)},
		{First: result.RowsAffected == 0, Second: durablejobexceptions.RoutineTag.NoChanges()},
	}); exception != nil {
		return exception
	}
