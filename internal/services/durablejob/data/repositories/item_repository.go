...
func (r *ItemRepository) CheckPermissionAndGetOneById(
	...
	if exception := exceptions.Cover(nil, []types.Pair[bool, *exceptions.Exception]{
		{First: result.Error != nil, Second: durablejobexceptions.Item.NotFound().WithOrigin(result.Error)},
		{First: item.Id == uuid.Nil, Second: durablejobexceptions.Item.NotFound()},
	}); exception != nil {
...
func (r *ItemRepository) CheckPermissionsAndGetManyByIds(
	...
	if exception := exceptions.Cover(nil, []types.Pair[bool, *exceptions.Exception]{
		{First: result.Error != nil, Second: durablejobexceptions.Item.NotFound().WithOrigin(result.Error)},
		{First: len(items) == 0, Second: durablejobexceptions.Item.NotFound()},
	}); exception != nil {
		return nil, exception
	}
...
func (r *ItemRepository) GetPermittedIdentities(
	...
	result := parsedOptions.DB.
		...
		Find(&permittedItems)
	if result.Error != nil {
		return nil, durablejobexceptions.Item.NotFound().WithOrigin(result.Error)
	}
