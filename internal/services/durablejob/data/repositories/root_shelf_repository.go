...
func (r *RootShelfRepository) CheckPermissionAndGetOneById(
	...
	if exception := exceptions.Cover(nil, []types.Pair[bool, *exceptions.Exception]{
		{First: result.Error != nil, Second: durablejobexceptions.Shelf.NotFound().WithOrigin(result.Error)},
		{First: rootShelf.Id == uuid.Nil, Second: durablejobexceptions.Shelf.NotFound()},
	}); exception != nil {
...
func (r *RootShelfRepository) CheckPermissionsAndGetManyByIds(
	...
	if exception := exceptions.Cover(nil, []types.Pair[bool, *exceptions.Exception]{
		{First: result.Error != nil, Second: durablejobexceptions.Shelf.NotFound().WithOrigin(result.Error)},
		{First: len(rootShelves) == 0, Second: durablejobexceptions.Shelf.NotFound()},
	}); exception != nil {
	...
	if allowedPermissions != nil {
		...
		if exception := exceptions.Cover(nil, []types.Pair[bool, *exceptions.Exception]{
			{First: result.Error != nil, Second: durablejobexceptions.Shelf.NotFound().WithOrigin(result.Error)},
			{First: len(usersToShelves) == 0, Second: durablejobexceptions.Shelf.NotFound()},
		}); exception != nil {
			return nil, nil, exception
		}
		...
		for index, rootShelf := range rootShelves {
			...
			if !exist {
				return nil, nil, durablejobexceptions.Shelf.NotFound()
			}
...
func (r *RootShelfRepository) CreateOne(
	...
	if err := copier.Copy(&newRootShelf, &input); err != nil {
		...
		return nil, durablejobexceptions.Shelf.FailedToCreate().WithOrigin(err)
	}
	...
	if err := result.Error; err != nil {
		...
		case "ERROR: duplicate key value violates unique constraint \"shelf_idx_owner_id_name\" (SQLSTATE 23505)":
			return nil, durablejobexceptions.Shelf.DuplicateName(input.Name)
		default:
			return nil, durablejobexceptions.Shelf.FailedToCreate().WithOrigin(err)
		}
	}
	...
	if exception := exceptions.Cover(nil, []types.Pair[bool, *exceptions.Exception]{
		{First: result.Error != nil, Second: durablejobexceptions.Shelf.FailedToCreate().WithOrigin(result.Error)},
		{First: result.RowsAffected == 0, Second: durablejobexceptions.Shelf.NoChanges()},
	}); exception != nil {
		parsedOptions.DB.Rollback()
		return nil, exception
	}
	...
	if shouldStartTransaction {
		if err := parsedOptions.DB.Commit().Error; err != nil {
			parsedOptions.DB.Rollback()
			return nil, durablejobexceptions.Shelf.FailedToCommitTransaction().WithOrigin(err)
		}
	}
...
func (r *RootShelfRepository) CreateMany(
	...
	if len(input) == 0 {
		return nil, durablejobexceptions.Shelf.NoChanges()
	}
	...
	for _, in := range input {
		...
		if err := copier.Copy(&newRootShelf, &in); err != nil {
			parsedOptions.DB.Rollback()
			return nil, durablejobexceptions.Shelf.InvalidDto().WithOrigin(err)
		}
...
func (r *RootShelfRepository) UpdateOneById(
	...
	result := parsedOptions.DB.Model(&schemas.RootShelf{}).
		...
		Updates(&updates)
	if exception := exceptions.Cover(nil, []types.Pair[bool, *exceptions.Exception]{
		{First: result.Error != nil, Second: durablejobexceptions.Shelf.FailedToUpdate().WithOrigin(result.Error)},
		{First: result.RowsAffected == 0, Second: durablejobexceptions.Shelf.NoChanges()},
	}); exception != nil {
		parsedOptions.DB.Rollback()
		return nil, exception
...
func (r *RootShelfRepository) UpdateManyByIds(
	...
	if parsedOptions.HasAllowedPermissions() {
		...
		if exception != nil {
			parsedOptions.DB.Rollback()
			return durablejobexceptions.Shelf.NoPermission("update these root shelves")
		}
	...
	if shouldStartTransaction {
		if err := parsedOptions.DB.Commit().Error; err != nil {
			parsedOptions.DB.Rollback()
			return durablejobexceptions.Shelf.FailedToCommitTransaction().WithOrigin(err)
		}
	}
...
func (r *RootShelfRepository) RestoreSoftDeletedOneById(
	...
	if exception := exceptions.Cover(nil, []types.Pair[bool, *exceptions.Exception]{
		{First: result.Error != nil, Second: durablejobexceptions.Shelf.FailedToUpdate().WithOrigin(result.Error)},
		{First: restoredRootShelf.Id == uuid.Nil, Second: durablejobexceptions.Shelf.FailedToUpdate()},
		{First: result.RowsAffected == 0, Second: durablejobexceptions.Shelf.NoChanges()},
	}); exception != nil {
		return nil, exception
	}
...
func (r *RootShelfRepository) RestoreSoftDeletedManyByIds(
	...
	if exception := exceptions.Cover(nil, []types.Pair[bool, *exceptions.Exception]{
		{First: result.Error != nil, Second: durablejobexceptions.Shelf.FailedToUpdate().WithOrigin(result.Error)},
		{First: len(restoredRootShelves) != len(ids), Second: durablejobexceptions.Shelf.FailedToUpdate()},
		{First: result.RowsAffected == 0, Second: durablejobexceptions.Shelf.NoChanges()},
	}); exception != nil {
		return nil, exception
	}
...
func (r *RootShelfRepository) SoftDeleteManyByIds(
	...
	if len(ids) == 0 {
		return durablejobexceptions.Shelf.NoChanges()
	}
...
func (r *RootShelfRepository) SoftDeleteManyByUserId(
	...
	if err := result.Error; err != nil {
		return durablejobexceptions.Shelf.FailedToDelete().WithOrigin(err)
	}
	if result.RowsAffected == 0 {
		return durablejobexceptions.Shelf.NotFound()
	}
...
func (r *RootShelfRepository) HardDeleteOneById(
	...
	result := query.
		...
		Delete(&schemas.RootShelf{})
	if exception := exceptions.Cover(nil, []types.Pair[bool, *exceptions.Exception]{
		{First: result.Error != nil, Second: durablejobexceptions.Shelf.FailedToDelete().WithOrigin(result.Error)},
		{First: result.RowsAffected == 0, Second: durablejobexceptions.Shelf.NoChanges()},
	}); exception != nil {
		return exception
	}
...
func (r *RootShelfRepository) BulkCheckPermissionsAndGetManyByIds(
	...
	if allowedPermissions != nil {
		...
		result := parsedOptions.DB.Model(&schemas.RootShelf{}).
			...
			Scan(&validTargets)
		if result.Error != nil {
			return nil, nil, durablejobexceptions.Shelf.NotFound().WithOrigin(result.Error)
		}
...
func (r *RootShelfRepository) BulkCreateMany(
	...
	if len(inputs) == 0 {
		return []bool{}, durablejobexceptions.Shelf.NoChanges()
	}
	...
	if result.Error != nil {
		parsedOptions.DB.Rollback()
		return nil, durablejobexceptions.Shelf.FailedToCreate().WithOrigin(result.Error)
	}
...
func (r *RootShelfRepository) BulkUpdateMany(
	...
	if result.Error != nil {
		parsedOptions.DB.Rollback()
		return nil, durablejobexceptions.Shelf.FailedToUpdate().WithOrigin(result.Error)
	}
