...
func (r *UsersToStationsRepository) GetOne(
	...
	result := parsedOptions.DB.
		...
		First(&relation)
	if result.Error != nil {
		return nil, durablejobexceptions.Station.NotFound().WithOrigin(result.Error)
	}
...
func (r *UsersToStationsRepository) UpsertMany(
	...
	if len(userIds) != len(permissions) {
		return nil, durablejobexceptions.Station.InvalidInput("userIds and permissions must have equal lengths")
	}
...
func (r *UsersToStationsRepository) UpdateOne(
	...
	if result.RowsAffected == 0 {
		return nil, durablejobexceptions.Station.NotFound()
	}
...
func (r *UsersToStationsRepository) DeleteOne(
	...
	if result.Error != nil {
		return durablejobexceptions.Station.FailedToDelete().WithOrigin(result.Error)
	}
	if result.RowsAffected == 0 {
		return durablejobexceptions.Station.NotFound()
	}
