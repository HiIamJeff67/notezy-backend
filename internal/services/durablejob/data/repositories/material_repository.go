...
func (r *MaterialRepository) CheckPermissionAndGetOneById(
	...
	if exception := exceptions.Cover(nil, []types.Pair[bool, *exceptions.Exception]{
		{First: result.Error != nil, Second: durablejobexceptions.Material.NotFound().WithOrigin(result.Error)},
		{First: material.Id == uuid.Nil, Second: durablejobexceptions.Material.NotFound()},
	}); exception != nil {
...
func (r *MaterialRepository) CheckPermissionsAndGetManyByIds(
	...
	if exception := exceptions.Cover(nil, []types.Pair[bool, *exceptions.Exception]{
		{First: result.Error != nil, Second: durablejobexceptions.Material.NotFound().WithOrigin(result.Error)},
		{First: len(materials) == 0, Second: durablejobexceptions.Material.NotFound()},
	}); exception != nil {
		return nil, exception
	}
...
func (r *MaterialRepository) CreateOneBySubShelfId(
	...
	if parsedOptions.HasAllowedPermissions() {
		...
		if !subShelfRepository.HasPermission(
			...
			return nil, durablejobexceptions.Shelf.NoPermission("create a material under this shelf")
		}
	}
	...
	if err := copier.Copy(&newMaterial, &input); err != nil {
		parsedOptions.DB.Rollback()
		return nil, durablejobexceptions.Material.FailedToCreate().WithOrigin(err)
	}
	...
	result := parsedOptions.DB.Model(&schemas.Material{}).
		Create(&newMaterial)
	if exception := exceptions.Cover(nil, []types.Pair[bool, *exceptions.Exception]{
		{First: result.Error != nil, Second: durablejobexceptions.Material.FailedToCreate().WithOrigin(result.Error)},
		{First: newMaterial.Id == uuid.Nil, Second: durablejobexceptions.Material.FailedToCreate()},
		{First: result.RowsAffected == 0, Second: durablejobexceptions.Material.NoChanges()},
	}); exception != nil {
		parsedOptions.DB.Rollback()
		return nil, exception
	}
	...
	if shouldStartTransaction {
		if err := parsedOptions.DB.Commit().Error; err != nil {
			parsedOptions.DB.Rollback()
			return nil, durablejobexceptions.Material.FailedToCommitTransaction().WithOrigin(err)
		}
	}
...
func (r *MaterialRepository) UpdateOneById(
	...
	if existingMaterial == nil {
		parsedOptions.DB.Rollback()
		return nil, durablejobexceptions.Material.NotFound()
	}
	...
	if input.Values.ParentSubShelfId != nil && !partialupdate.CheckSetNull(input.SetNull, "ParentSubShelfId") {
		...
		if !subShelfRepository.HasPermission(
			*input.Values.ParentSubShelfId,
			...
			parsedOptions.DB.Rollback()
			return nil, durablejobexceptions.Shelf.NoPermission("move a material to this shelf")
		}
	}
	...
	result := parsedOptions.DB.Model(&schemas.Material{}).
		...
		Updates(&updates)
	if exception := exceptions.Cover(nil, []types.Pair[bool, *exceptions.Exception]{
		{First: result.Error != nil, Second: durablejobexceptions.Material.FailedToUpdate().WithOrigin(result.Error)},
		{First: result.RowsAffected == 0, Second: durablejobexceptions.Material.NoChanges()},
	}); exception != nil {
		parsedOptions.DB.Rollback()
		return nil, exception
...
func (r *MaterialRepository) RestoreSoftDeletedOneById(
	...
	if exception := exceptions.Cover(nil, []types.Pair[bool, *exceptions.Exception]{
		{First: result.Error != nil, Second: durablejobexceptions.Material.FailedToUpdate().WithOrigin(result.Error)},
		{First: restoredMaterial.Id == uuid.Nil, Second: durablejobexceptions.Material.FailedToUpdate()},
		{First: result.RowsAffected == 0, Second: durablejobexceptions.Material.NoChanges()},
	}); exception != nil {
		return nil, exception
	}
...
func (r *MaterialRepository) RestoreSoftDeletedManyByIds(
	...
	if len(ids) == 0 {
		return nil, durablejobexceptions.Material.NoChanges()
	}
	...
	if exception := exceptions.Cover(nil, []types.Pair[bool, *exceptions.Exception]{
		{First: result.Error != nil, Second: durablejobexceptions.Material.FailedToUpdate().WithOrigin(result.Error)},
		{First: len(restoredMaterials) != len(ids), Second: durablejobexceptions.Material.FailedToUpdate()},
		{First: result.RowsAffected == 0, Second: durablejobexceptions.Material.NoChanges()},
	}); exception != nil {
		return nil, exception
	}
...
func (r *MaterialRepository) SoftDeleteManyByIds(
	...
	if len(ids) == 0 {
		return durablejobexceptions.Material.NoChanges()
	}
...
func (r *MaterialRepository) HardDeleteOneById(
	...
	if exception := exceptions.Cover(nil, []types.Pair[bool, *exceptions.Exception]{
		{First: result.Error != nil, Second: durablejobexceptions.Material.FailedToDelete().WithOrigin(result.Error)},
		{First: result.RowsAffected == 0, Second: durablejobexceptions.Material.NoChanges()},
	}); exception != nil {
		return exception
	}
...
func (r *MaterialRepository) BulkCheckPermissionsAndGetManyByIds(
	...
	if allowedPermissions != nil {
		...
		result := parsedOptions.DB.Model(&schemas.Material{}).
			...
			Scan(&validTargets)
		if result.Error != nil {
			return nil, nil, durablejobexceptions.Material.NotFound().WithOrigin(result.Error)
		}
...
func (r *MaterialRepository) BulkDeleteMany(
	...
	if len(bulkInputs) == 0 {
		return []bool{}, durablejobexceptions.Material.NoChanges()
	}
	...
	if result.Error != nil {
		parsedOptions.DB.Rollback()
		return nil, durablejobexceptions.Material.FailedToDelete().WithOrigin(result.Error)
	}
