...
func (r *RoutineTaskRepository) CheckPermissionAndGetOneById(
	...
	if exception := exceptions.Cover(nil, []types.Pair[bool, *exceptions.Exception]{
		{First: result.Error != nil, Second: durablejobexceptions.RoutineTask.NotFound().WithOrigin(result.Error)},
		{First: routineTask.Id == uuid.Nil, Second: durablejobexceptions.RoutineTask.NotFound()},
	}); exception != nil {
...
func (r *RoutineTaskRepository) CheckPermissionsAndGetManyByIds(
	...
	if exception := exceptions.Cover(nil, []types.Pair[bool, *exceptions.Exception]{
		{First: result.Error != nil, Second: durablejobexceptions.RoutineTask.NotFound().WithOrigin(result.Error)},
		{First: len(routineTasks) == 0, Second: durablejobexceptions.RoutineTask.NotFound()},
	}); exception != nil {
		return nil, exception
	}
...
func (r *RoutineTaskRepository) GetAllByUserId(
	...
	result := parsedOptions.DB.
		...
		Find(&routineTasks)
	if result.Error != nil {
		return nil, durablejobexceptions.RoutineTask.NotFound().WithOrigin(result.Error)
	}
...
func (r *RoutineTaskRepository) CreateOneByRoutineId(
	...
	if input.ActorUserId == uuid.Nil || input.ActorUserId != userId {
		return nil, durablejobexceptions.RoutineTask.InvalidInput().WithOrigin(fmt.Errorf("actorUserId must match userId"))
	}
	...
	if !routineRepository.HasPermission(
		...
		return nil, durablejobexceptions.RoutineTask.NoPermission("create a routine task under this routine")
	}

	if input.NextScheduledAt.IsZero() {
		parsedOptions.DB.Rollback()
		return nil, durablejobexceptions.RoutineTask.InvalidInput().WithOrigin(fmt.Errorf("nextScheduledAt is required"))
	}

	newRoutineTask := schemas.RoutineTask{}
	if err := copier.Copy(&newRoutineTask, &input); err != nil {
		parsedOptions.DB.Rollback()
		return nil, durablejobexceptions.RoutineTask.InvalidInput().WithOrigin(err)
	}
	...
	if exception := exceptions.Cover(nil, []types.Pair[bool, *exceptions.Exception]{
		{First: result.Error != nil, Second: durablejobexceptions.RoutineTask.FailedToCreate().WithOrigin(result.Error)},
		{First: result.RowsAffected == 0, Second: durablejobexceptions.RoutineTask.NoChanges()},
	}); exception != nil {
		parsedOptions.DB.Rollback()
		return nil, exception
	}
	...
	if shouldStartTransaction {
		if err := parsedOptions.DB.Commit().Error; err != nil {
			parsedOptions.DB.Rollback()
			return nil, durablejobexceptions.RoutineTask.FailedToCommitTransaction().WithOrigin(err)
		}
	}
...
func (r *RoutineTaskRepository) CreateManyByRoutineIds(
	...
	if len(input) == 0 {
		return nil, durablejobexceptions.RoutineTask.NoChanges()
	}
...
func (r *RoutineTaskRepository) UpdateOneById(
	...
	if input.Values.RoutineId != nil && !partialupdate.CheckSetNull(input.SetNull, "RoutineId") {
		...
		if !routineRepository.HasPermission(*input.Values.RoutineId, userId, parsedOptions.AllowedPermissions, opts...) {
			parsedOptions.DB.Rollback()
			return nil, durablejobexceptions.RoutineTask.NoPermission("move a routine task to this routine")
		}
	}
	...
	result := parsedOptions.DB.
		...
		Updates(&updates)
	if exception := exceptions.Cover(nil, []types.Pair[bool, *exceptions.Exception]{
		{First: result.Error != nil, Second: durablejobexceptions.RoutineTask.FailedToUpdate().WithOrigin(result.Error)},
		{First: result.RowsAffected == 0, Second: durablejobexceptions.RoutineTask.NoChanges()},
	}); exception != nil {
		parsedOptions.DB.Rollback()
		return nil, exception
...
func (r *RoutineTaskRepository) UpdateManyByIds(
	...
	if len(input) == 0 {
		return durablejobexceptions.RoutineTask.NoChanges()
	}
	...
	if exception != nil {
		parsedOptions.DB.Rollback()
		return durablejobexceptions.RoutineTask.NoPermission("update these routine tasks")
	}
	...
	if len(targetRoutineIdSet) > 0 {
		...
		if !routineRepository.HavePermissions(targetRoutineIds, userId, parsedOptions.AllowedPermissions, opts...) {
			parsedOptions.DB.Rollback()
			return durablejobexceptions.RoutineTask.NoPermission("move these routine tasks to the given routines")
		}
	}
	...
	if shouldStartTransaction {
		if err := parsedOptions.DB.Commit().Error; err != nil {
			parsedOptions.DB.Rollback()
			return durablejobexceptions.RoutineTask.FailedToCommitTransaction().WithOrigin(err)
		}
	}
...
func (r *RoutineTaskRepository) HardDeleteOneById(
	...
	if exception := exceptions.Cover(nil, []types.Pair[bool, *exceptions.Exception]{
		{First: result.Error != nil, Second: durablejobexceptions.RoutineTask.FailedToDelete().WithOrigin(result.Error)},
		{First: result.RowsAffected == 0, Second: durablejobexceptions.RoutineTask.NoChanges()},
	}); exception != nil {
		return exception
	}
