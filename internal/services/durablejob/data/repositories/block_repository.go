...
func (r *BlockRepository) CheckPermissionAndGetOneById(
	...
	if exception := exceptions.Cover(nil, []types.Pair[bool, *exceptions.Exception]{
		{First: result.Error != nil, Second: durablejobexceptions.Block.NotFound().WithOrigin(result.Error)},
		{First: block.Id == uuid.Nil, Second: durablejobexceptions.Block.NotFound()},
	}); exception != nil {
...
func (r *BlockRepository) CheckPermissionsAndGetManyByIds(
	...
	if exception := exceptions.Cover(nil, []types.Pair[bool, *exceptions.Exception]{
		{First: result.Error != nil, Second: durablejobexceptions.Block.NotFound().WithOrigin(result.Error)},
		{First: len(blocks) == 0, Second: durablejobexceptions.Block.NotFound()},
	}); exception != nil {
		return nil, exception
	}
...
func (r *BlockRepository) BulkCheckPermissionsAndGetManyByIds(
	...
	result := parsedOptions.DB.Model(&schemas.Block{}).
		...
		Scan(&validTargets)
	if result.Error != nil {
		return nil, nil, durablejobexceptions.Block.NotFound().WithOrigin(result.Error)
	}
...
func (r *BlockRepository) BulkCreateMany(
	...
	if len(bulkInputs) == 0 {
		return []bool{}, durablejobexceptions.Block.NoChanges()
	}
	...
	if result.Error != nil {
		...
		return nil, durablejobexceptions.Block.FailedToCreate().WithOrigin(result.Error)
	}
	...
	if shouldStartTransaction {
		if err := parsedOptions.DB.Commit().Error; err != nil {
			...
			return nil, durablejobexceptions.Block.FailedToCommitTransaction().WithOrigin(err)
		}
	}
...
func (r *BlockRepository) BulkUpdateMany(
	...
	if result.Error != nil {
		...
		return nil, durablejobexceptions.Block.FailedToUpdate().WithOrigin(result.Error)
	}
