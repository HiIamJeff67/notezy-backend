...
func (r *BlockPackRepository) CheckPermissionAndGetOneById(
	...
	if exception := exceptions.Cover(nil, []types.Pair[bool, *exceptions.Exception]{
		{First: result.Error != nil, Second: durablejobexceptions.BlockPack.NotFound().WithOrigin(result.Error)},
		{First: blockPack.Id == uuid.Nil, Second: durablejobexceptions.BlockPack.NotFound()},
	}); exception != nil {
...
func (r *BlockPackRepository) CheckPermissionsAndGetManyByIds(
	...
	if exception := exceptions.Cover(nil, []types.Pair[bool, *exceptions.Exception]{
		{First: result.Error != nil, Second: durablejobexceptions.BlockPack.NotFound().WithOrigin(result.Error)},
		{First: len(blockPacks) == 0, Second: durablejobexceptions.BlockPack.NotFound()},
	}); exception != nil {
		return nil, exception
	}
...
func (r *BlockPackRepository) CheckPermissionAndGetOneWithOwnerIdById(
	...
	if exception := exceptions.Cover(nil, []types.Pair[bool, *exceptions.Exception]{
		{First: result.Error != nil, Second: durablejobexceptions.BlockPack.NotFound().WithOrigin(result.Error)},
		{First: blockPackWithOwnerId.OwnerId == uuid.Nil, Second: durablejobexceptions.BlockPack.NotFound()},
	}); exception != nil {
...
func (r *BlockPackRepository) CheckPermissionsAndGetManyWithOwnerIdsByIds(
	...
	if exception := exceptions.Cover(nil, []types.Pair[bool, *exceptions.Exception]{
		{First: result.Error != nil, Second: durablejobexceptions.BlockPack.NotFound().WithOrigin(result.Error)},
		{First: len(blockPacksWithOwnerIds) == 0, Second: durablejobexceptions.BlockPack.NotFound()},
	}); exception != nil {
		return nil, nil, exception
	}
...
func (r *BlockPackRepository) GetManyByRootShelfIds(
	...
	result := parsedOptions.DB.
		...
		Find(&blockPacks)
	if result.Error != nil {
		return nil, durablejobexceptions.BlockPack.NotFound().WithOrigin(result.Error)
	}
...
func (r *BlockPackRepository) CreateOneBySubShelfId(
	...
	if parsedOptions.HasAllowedPermissions() {
		...
		if !subShelfRepository.HasPermission(
			...
			return nil, durablejobexceptions.Shelf.NoPermission("create a block pack under this shelf")
		}
	}
	...
	if err := copier.Copy(&newBlockPack, &input); err != nil {
		parsedOptions.DB.Rollback()
		return nil, durablejobexceptions.BlockPack.FailedToCreate().WithOrigin(err)
	}
	...
	result := parsedOptions.DB.Model(&schemas.BlockPack{}).
		Create(&newBlockPack)
	if exception := exceptions.Cover(nil, []types.Pair[bool, *exceptions.Exception]{
		{First: result.Error != nil, Second: durablejobexceptions.BlockPack.FailedToCreate().WithOrigin(result.Error)},
		{First: newBlockPack.Id == uuid.Nil, Second: durablejobexceptions.BlockPack.FailedToCreate()},
		{First: result.RowsAffected == 0, Second: durablejobexceptions.BlockPack.NoChanges()},
	}); exception != nil {
		parsedOptions.DB.Rollback()
		return nil, exception
	}
	...
	if shouldStartTransaction {
		if err := parsedOptions.DB.Commit().Error; err != nil {
			parsedOptions.DB.Rollback()
			return nil, durablejobexceptions.BlockPack.FailedToCommitTransaction().WithOrigin(err)
		}
	}
...
func (r *BlockPackRepository) CreateManyBySubShelfIds(
	...
	for _, in := range input {
		...
		if err := copier.Copy(&newBlockPack, &in); err != nil {
			parsedOptions.DB.Rollback()
			return nil, durablejobexceptions.BlockPack.InvalidInput().WithOrigin(err)
		}
		if newBlockPack.Id == uuid.Nil {
			newBlockPack.Id = uuid.New()
		}
	...
	if exception := exceptions.Cover(nil, []types.Pair[bool, *exceptions.Exception]{
		{First: result.Error != nil, Second: durablejobexceptions.Block.FailedToCreate().WithOrigin(result.Error)},
		{First: result.RowsAffected == 0, Second: durablejobexceptions.BlockPack.NoChanges()},
	}); exception != nil {
		parsedOptions.DB.Rollback()
		return nil, exception
...
func (r *BlockPackRepository) UpdateOneById(
	...
	if input.Values.ParentSubShelfId != nil && !partialupdate.CheckSetNull(input.SetNull, "ParentSubShelfId") {
		...
		if !subShelfRepository.HasPermission(
			*input.Values.ParentSubShelfId,
			...
			parsedOptions.DB.Rollback()
			return nil, durablejobexceptions.Shelf.NoPermission("move a block pack to this shelf")
		}
	}
	...
	result := parsedOptions.DB.Model(&schemas.BlockPack{}).
		...
		Updates(&updates)
	if exception := exceptions.Cover(nil, []types.Pair[bool, *exceptions.Exception]{
		{First: result.Error != nil, Second: durablejobexceptions.BlockPack.FailedToUpdate().WithOrigin(result.Error)},
		{First: result.RowsAffected == 0, Second: durablejobexceptions.BlockPack.NoChanges()},
	}); exception != nil {
		parsedOptions.DB.Rollback()
		return nil, exception
...
func (r *BlockPackRepository) UpdateManyByIds(
	...
	if shouldStartTransaction {
		if err := parsedOptions.DB.Commit().Error; err != nil {
			parsedOptions.DB.Rollback()
			return durablejobexceptions.BlockPack.FailedToCommitTransaction().WithOrigin(err)
		}
	}
...
func (r *BlockPackRepository) RestoreSoftDeletedManyByIds(
	...
	if len(ids) == 0 {
		return nil, durablejobexceptions.BlockPack.NoChanges()
	}
...
func (r *BlockPackRepository) SoftDeleteOneById(
	...
	if exception := exceptions.Cover(nil, []types.Pair[bool, *exceptions.Exception]{
		{First: result.Error != nil, Second: durablejobexceptions.BlockPack.FailedToDelete().WithOrigin(result.Error)},
		{First: result.RowsAffected == 0, Second: durablejobexceptions.BlockPack.NoChanges()},
	}); exception != nil {
		return exception
	}
...
func (r *BlockPackRepository) SoftDeleteManyByIds(
	...
	if len(ids) == 0 {
		return durablejobexceptions.BlockPack.NoChanges()
	}
...
func (r *BlockPackRepository) BulkCheckPermissionsAndGetManyByIds(
	...
	if allowedPermissions != nil {
		...
		result := parsedOptions.DB.Model(&schemas.BlockPack{}).
			...
			Scan(&validTargets)
		if result.Error != nil {
			return nil, nil, durablejobexceptions.BlockPack.NotFound().WithOrigin(result.Error)
		}
...
func (r *BlockPackRepository) BulkCreateMany(
	...
	if len(inputs) == 0 {
		return []bool{}, durablejobexceptions.BlockPack.NoChanges()
	}
	...
	if result.Error != nil {
		parsedOptions.DB.Rollback()
		return nil, durablejobexceptions.BlockPack.FailedToCreate().WithOrigin(result.Error)
	}
...
func (r *BlockPackRepository) BulkUpdateMany(
	...
	if len(targetSubShelfIds) > 0 {
		...
		if result.Error != nil {
			parsedOptions.DB.Rollback()
			return nil, durablejobexceptions.BlockPack.FailedToUpdate().WithOrigin(result.Error)
		}
...
func (r *BlockPackRepository) BulkDeleteMany(
	...
	if result.Error != nil {
		parsedOptions.DB.Rollback()
		return nil, durablejobexceptions.BlockPack.FailedToDelete().WithOrigin(result.Error)
	}
