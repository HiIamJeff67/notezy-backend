...
func (c *DurableJobRoutineTaskResultConsumer) consume(
	...
	if event.EventType != coreeventscontract.EventType_RoutineTasksCompleted && event.EventType != coreeventscontract.EventType_RoutineTasksFailed {
		return nil
	}

	if event.EventId == uuid.Nil || event.AggregateType != coreeventscontract.AggregateType_DurableJobWorker || event.AggregateId == uuid.Nil || event.KafkaKey != event.AggregateId.String() {
	...
	case coreeventscontract.EventType_RoutineTasksCompleted:
		...
		if exception := c.routineTaskService.MarkCompletedRoutineTasks(ctx, event.EventId, &request); exception != nil {
			return &platformkafka.ConsumerError{Classification: platformkafka.ErrorClassification_Transient, Origin: exception}
		}
	case coreeventscontract.EventType_RoutineTasksFailed:
