...
func (c *YjsMaintenanceRequestConsumer) consume(
	...
	if request.RequestId == uuid.Nil || request.BlockPackId != event.AggregateId || request.DocumentId == uuid.Nil ||
		(request.Operation != durablejobeventscontract.YjsMaintenanceOperation_Compact && request.Operation != durablejobeventscontract.YjsMaintenanceOperation_Project) ||
	...
	if (request.Operation == durablejobeventscontract.YjsMaintenanceOperation_Compact &&
		targetSequence <= document.CompactedUntilSequence) ||
		(request.Operation == durablejobeventscontract.YjsMaintenanceOperation_Project &&
...
func (c *YjsMaintenanceRequestConsumer) publishResult(
	...
	if err := c.resultProducer.Produce(ctx, event, durablejobeventscontract.YjsMaintenanceResultData{
