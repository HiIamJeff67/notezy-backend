...
func TestLoadConfig(t *testing.T) {
	...
	t.Setenv("EMAIL_SERVICE_BASE_URL", "http://email:8081")
	t.Setenv("EMAIL_SERVICE_CLIENT_TIMEOUT", "5s")

	config, err := LoadConfig()
	if err != nil {
		t.Fatalf("LoadConfig() error = %v", err)
	}
	if config.EmailClientTimeout != 5*time.Second || config.OutboxRelay.BatchSize != 100 {
