...
type SubShelf struct {
	...
	Path           types.UUIDArray `json:"path" gorm:"column:path; type:uuid[]; not null; default:'{}'; check:path_length_check,cardinality(path) >= 0 AND cardinality(path) <= 100; uniqueIndex:sub_shelf_idx_name_root_shelf_id_path;"`
