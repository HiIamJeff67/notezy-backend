...
func init() {
    ...
    colog.Register() // 註冊 colog，讓它接管內建的 log
    colog.SetMinLevel(colog.LInfo) // 設定最小日誌等級為 Info
    colog.SetDefaultLevel(colog.LInfo) // 預設日誌等級為 Info
    colog.ParseFields(false) // 不解析日誌中的欄位
}
