...
- 檔名採 `snake_case.go`，並依功能命名，例如 `root_shelf_service.go`、`root_shelf_binder_test.go`。
- package 名稱維持小寫單數或既有複數慣例；不要為單一用途建立新 package。
- import 由 `gofmt` 排序，標準函式庫、第三方、專案內 import 以空行分組。專案內 package 保留既有明確別名，例如 `dtos`、`schemas`、`exceptions`、`types`。
...
- 與外部 API 對應的 request/response 使用 `XxxReqDto`、`XxxResDto`；資料庫寫入資料使用 `XxxInput`；資料庫 table 使用 `schemas.Xxx`。
- 新功能以領域名對齊檔案、module、binder、controller、service、repository、scope，例如 `station_*`。
...
- `app/models/inputs` 的 `XxxInput` 是 repository persistence contract：描述 create、update、partial update 或 bulk SQL 所需的資料，只能作為 repository method 的 input。service、controller、binder 或 gateway 不得把它當成外部 request/response contract。
- service method 若參數很多、代表同一個完整意圖，或由 HTTP/realtime gateway 接收後要輸出到外部，就使用 `*dtos.XxxReqDto` 作為單一 request parameter。HTTP request 沿用 `reqDto` 命名，並以 `*dtos.XxxResDto` / `resDto` 作為回傳資料。
- service-only 或 gateway-only workflow 也可使用具體的 `XxxReqDto` / `XxxResDto` 封裝相關資料、context 與輸出，而不是用長串零散 parameters 或 anonymous struct；DTO 名稱必須描述操作，不可使用泛用的 `Data`、`Params` 或 `Payload`。
- 參數少且語意清楚時保留直接參數，不為了套用 DTO 強行包裝。只有 parameter 數量、共同 lifecycle 或呼叫邊界確實使 DTO 提高可讀性時才建立。
- service 是兩種 contract 的轉換邊界：將 `reqDto` 的已驗證資料映射為 `inputs.CreateXxxInput`、`inputs.PartialUpdateXxxInput` 或 bulk input，再呼叫 repository；repository 不得 import 或依賴 `dtos`。
...
- 函式只負責一個層級的工作。HTTP parsing 留在 binder，HTTP response 留在 controller，流程與交易留在 service，查詢與權限 SQL 留在 repository/scope。
...
- 依問題選用既有 package：去重/set 使用 `shared/lib/array`，游標分頁使用 `shared/lib/searchcursor`，併發工作使用 `shared/lib/concurrency`，佇列與堆疊使用 `shared/lib/queue`、`shared/lib/stack`，以及其他已存在的 blocknote、rate-limit、response writer 函式庫。
- `shared/lib` 僅放跨領域、可重用且與 application layer 無關的邏輯。僅由單一領域使用的商業規則留在該領域，不要為了「可能重用」移入 shared。
