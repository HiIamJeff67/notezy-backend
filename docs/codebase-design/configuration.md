...
| OpenTelemetry | `shared/platform/observability/config.go` | `OTEL_SERVICE_*`, `OTEL_EXPORTER_OTLP_GRPC_ENDPOINT` |
| Gateway | `internal/gateway/configs/` | `GATEWAY_LISTEN_ADDRESS`, `CORE_BASE_URL`, `REALTIME_GATEWAY_BASE_URL`, Gateway Redis cache ranges |
